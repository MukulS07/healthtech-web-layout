...
const postData: Record<string, BlogPost> = {
  "piles-surgery-guide": {
    ...
    read: "5 min read",
    excerpt:
      "Not every case of piles needs surgery. Here's how specialists decide when laser treatment is the best option.",
    body: [
      {
      ...
      {
        heading: "When do piles not need surgery?",
        text: "Grade 1 and early Grade 2 piles often respond well to dietary changes and topical treatments. Increasing fibre intake, staying hydrated and avoiding prolonged sitting on the toilet can resolve mild haemorrhoids completely. We always explore conservative management first.",
      },
      {
        heading: "The tipping point: when is surgery indicated?",
        text: "Surgery becomes the right conversation when haemorrhoids are Grade 2 or above and don't respond to medication, when there is persistent bleeding (even if not heavy), when pain interrupts daily life, or when a prolapsed haemorrhoid cannot be pushed back manually.",
      },
      {
      ...
      },
      {
        ...
        text: "Under short general anaesthesia (or spinal, depending on the case), a small fibre-optic probe delivers laser energy precisely to the haemorrhoidal tissue. The whole procedure takes around 20–30 minutes. There are no stitches, no dressings and no dietary restrictions beyond the first 24 hours.",
      },
      {
    ...
    related: [
      {
        title: "Anal Fissure Treatment: what patients need to know",
        slug: "anal-fissure-guide",
        cat: "Proctology",
        read: "4 min read",
      },
      {
        title: "What to eat in the first week after colorectal surgery",
        slug: "post-colorectal-diet",
        cat: "Nutrition",
        read: "3 min read",
      },
      {
...
const fallbackPost: BlogPost = {
  ...
  body: [
    ...
    },
    {
      ...
      text: "Your specialist will walk through every aspect of the procedure during the free first consultation. Feel free to ask about anaesthesia, expected recovery time, cost and insurance coverage.",
    },
    {
      heading: "On surgery day",
      text: "Our care coordinator will be with you from the moment you arrive. We manage all hospital paperwork and insurance pre-authorisation so you can focus entirely on your health.",
    },
    {
      heading: "Recovery",
  ...
  related: [
    ...
    {
      title: "Pre-surgery checklist: 14 things to do before your operation",
      ...
      <main>
        <nav className="border-b border-border bg-cream py-3 text-xs text-muted-foreground">
          <Container className="flex items-center gap-2">
            ...
            <span>/</span>
            <a href="/blog" className="hover:text-brand-orange">
              Healthfeed
            </a>
            <span>/</span>
        ...
        <Container className="grid gap-10 py-12 lg:grid-cols-[1.4fr_0.6fr]">
          ...
          <article className="min-w-0">
            ...
            <h1 className="mt-3 text-2xl font-bold leading-snug text-navy sm:text-3xl lg:text-4xl">
              {post.title}
            </h1>
            ...
            <div className="mt-10 flex flex-wrap gap-2 border-t border-border pt-6">
              {post.tags.map((t) => (
                <span
                  key={t}
                  className="rounded-full bg-cream px-3 py-1.5 text-xs font-medium text-ink/70 border border-border"
                >
                  {t}
                </span>
              ))}
                ...
                <div className="mt-4 space-y-3">
                  {post.related.map((r) => (
                    <a
                      key={r.slug}
                      href={`/blog/${r.slug}`}
                      className="group flex items-center justify-between gap-4 rounded-lg border border-border bg-cream p-4 transition-shadow hover:shadow-sm"
                    >
                      <div>
                        <span className="text-[11px] font-semibold text-brand-orange">{r.cat}</span>
                        <p className="mt-0.5 text-sm font-semibold text-navy group-hover:text-brand-orange transition-colors">
                          {r.title}
                        </p>
